...
const DOCKING_RANGE = 800; // Increased for easier approach
...
const ProximityBody: React.FC<{ 
    ...
    return (
        <group rotation={[0, 0, inclination]}>
            ...
            <group ref={groupRef}>
                <mesh>
                     {isStation ? (<octahedronGeometry args={[size, 0]} />) : (<sphereGeometry args={[size, 32, 32]} />)}
                     <meshStandardMaterial color={color} roughness={0.7} metalness={0.2} emissive={isStation ? color : '#000'} emissiveIntensity={isStation ? 0.5 : 0} />
                </mesh>
                {node.type === 'PLANET' && (
                    <mesh scale={[1.1, 1.1, 1.1]}><sphereGeometry args={[size, 32, 32]} /><meshBasicMaterial color={color} transparent opacity={0.1} side={THREE.BackSide} /></mesh>
                )}
                <Billboard position={[0, size + 100, 0]}>
                    <Text fontSize={80} color="#e2e8f0" outlineWidth={2} outlineColor="#000000">{node.name}<meshBasicMaterial depthTest={false} /></Text>
                    <Text position={[0, -50, 0]} fontSize={30} color="#94a3b8">{Math.round(orbitRadius)}u</Text>
                </Billboard>
                {node.children && node.children.map((moon, i) => {
                     const moonOffset = (i / childrenCount) * Math.PI * 2;
                     const moonOrbit = size * 4 + (i * size * 2.5);
                     return (
                        <ProximityBody key={moon.id} node={moon} orbitRadius={moonOrbit} speed={speed * 2.5} offset={moonOffset} shipPosition={shipPosition} onDockingRange={onDockingRange} parentRef={groupRef} inclination={(i % 2 === 0 ? 0.2 : -0.2) * (i + 1)} isTarget={dockingTargetId === moon.id} dockingTargetId={dockingTargetId} targetDynamicsRef={targetDynamicsRef} />
                    );
                })}
            </group>
        </group>
...
export const SolarSystem: React.FC<SolarSystemProps> = ({ system, shipPosition, onDockingTargetChange, dockingTargetId, targetDynamicsRef, visible }) => {
    ...
    return (
        <group>
            <mesh><sphereGeometry args={[STAR_RADIUS, 64, 64]} /><meshBasicMaterial color={system.color || "#fbbf24"} /></mesh>
            <mesh><sphereGeometry args={[STAR_RADIUS * 1.2, 32, 32]} /><meshBasicMaterial color={system.color || "#fbbf24"} transparent opacity={0.2} side={THREE.BackSide} /></mesh>
            <pointLight color={system.color || "#fbbf24"} intensity={2.5} distance={20000} decay={1} />
            <Billboard><Text position={[0, STAR_RADIUS + 100, 0]} fontSize={120} color={system.color || "#fbbf24"}>{system.name}</Text></Billboard>
            {system.children?.map((child, index) => {
                const orbitRadius = STAR_RADIUS + 2500 + (index * AU);
                const speed = ORBIT_SPEED_BASE / (1 + index * 0.2);
                const offset = (index / childrenCount) * Math.PI * 2;
                const inclination = (index % 2 === 0 ? 1 : -1) * (0.05 + (index * 0.02));
                return (
                    <ProximityBody key={child.id} node={child} orbitRadius={orbitRadius} speed={speed} offset={offset} shipPosition={shipPosition} onDockingRange={handleDockingReport} inclination={inclination} isTarget={dockingTargetId === child.id} dockingTargetId={dockingTargetId} targetDynamicsRef={targetDynamicsRef} />
                );
            })}
        </group>
    );
