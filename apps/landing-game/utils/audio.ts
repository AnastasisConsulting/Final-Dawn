...
export class EngineSound {
  ...
  update(throttle: number, boosting: boolean) {
    ...
    const totalSpeed = Math.min(1, throttle + boostAmt * 0.5);

    // RUMBLE
    // Pitch goes up slightly, volume goes up
    this.rumbleOsc.frequency.setTargetAtTime(40 + (totalSpeed * 40), t, 0.1);
    this.rumbleGain!.gain.setTargetAtTime(0.2 + (totalSpeed * 0.2), t, 0.1);

    // WIND (Always present based on speed)
    // Filter opens up as you go faster
    this.windFilter!.frequency.setTargetAtTime(1000 - (totalSpeed * 400), t, 0.1); // Lower cutoff allows more mid-range wind
    this.windGain!.gain.setTargetAtTime(totalSpeed * 0.4, t, 0.1);

    // BOOST (The heavy thruster sound)
    if (boosting) {
        this.boostGain!.gain.setTargetAtTime(0.5, t, 0.2);
        this.boostFilter!.frequency.setTargetAtTime(400, t, 0.2); // Open up the roar
    } else {
        this.boostGain!.gain.setTargetAtTime(0.0, t, 0.5); // Fade out
        this.boostFilter!.frequency.setTargetAtTime(100, t, 0.5);
    }
  }
