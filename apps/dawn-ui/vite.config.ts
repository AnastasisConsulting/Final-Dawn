...
export default defineConfig(({ mode }) => {
  ...
  const generateCityMap = (seed: string, worldId: string, civId: string, ctIndex: number, gridSize = 7) => {
    const seedFn = xmur3(seed);
    const rng = mulberry32(seedFn());
  ...
  return {
    server: {
      port: 3001,
      host: '0.0.0.0',
      // Dawn-UI imports Flight-One source directly (../flight-one-weapons/*).
      // Vite blocks serving files outside the project root unless allowed.
      fs: {
        allow: [
          path.resolve(__dirname, '..'),
          path.resolve(__dirname, '../flight-one'),
          path.resolve(__dirname, '../flight-one-weapons'),
          path.resolve(__dirname, '../memory-viz'),
          ...
          server.middlewares.use('/api/topography', async (req, res) => {
            ...
            try {
              ...
              if (fs.existsSync(topoPath)) {
                ...
                const sector = JSON.parse(fs.readFileSync(sectorPath, 'utf8'));
          ...
          server.middlewares.use('/api/city-map', async (req, res) => {
            ...
            try {
              ...
              if (fs.existsSync(cityPath)) {
                cityData = JSON.parse(fs.readFileSync(cityPath, 'utf8'));
              }
    ...
    resolve: {
      alias: {
        ...
        '@eideus/memory-lattice-api': path.resolve(__dirname, '../../packages/eideus-memory-lattice-api/src'),
        'eideus-universe-mapper': path.resolve(__dirname, '../../packages/eideus-universe-mapper/src'),
        'eideus-world-bundle-binder': path.resolve(__dirname, '../../packages/eideus-world-bundle-binder/src'),
        'eideus-memory-lattice-api': path.resolve(__dirname, '../../packages/eideus-memory-lattice-api/src'),
