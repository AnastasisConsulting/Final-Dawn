...
import { RingConfig, Transform, Vector3 } from '../../../../types/vizzy';
...
const ComplexRing = ({ name, radius, thickness, segments, axis, speed, config, offset, transform }: ComplexRingProps) => {
    ...
    useFrame((state) => {
        ...
        if (innerTrack.current) innerTrack.current.rotation.z = -t * 2;
        if (outerRail.current) outerRail.current.rotation.z = t * 0.5;
        if (pistonsRef.current) {
            pistonsRef.current.children.forEach((child, i) => {
                const shift = Math.sin(t * 5 + i) * (thickness * 0.5);
                child.position.z = shift * 0.1;
                child.scale.set(1, 1, 1 + Math.sin(t * 10 + i) * 0.1);
            });
        }
    });
