...
const FriendlyShipGeometry = () => (
    <group rotation={[0, Math.PI, 0]}>
        <mesh><boxGeometry args={[12, 12, 40]} /><meshStandardMaterial color="#334155" roughness={0.7} /></mesh>
        <pointLight position={[0, 5, -15]} color="#22d3ee" distance={50} intensity={2} />
    </group>
);

const EnemyShipGeometry = ({ tier }: { tier: TargetTier }) => {
    // VISUAL UPGRADE: Replaced primitives with sci-fi shapes
    
    if (tier === 'BOSS') {
        // Boss: "Dreadnought" - Large Wedge
        return (
            <group rotation={[0, Math.PI, 0]}>
                {/* Main Wedge */}
                <mesh scale={[1, 0.3, 2]}>
                ...
                {/* Glow Engines */}
                <mesh position={[10, 0, 40]} rotation={[Math.PI/2, 0, 0]}>
                     <cylinderGeometry args={[4, 6, 5, 16]} />
                     <meshStandardMaterial emissive="#a855f7" emissiveIntensity={2} color="#000" />
                </mesh>
                <mesh position={[-10, 0, 40]} rotation={[Math.PI/2, 0, 0]}>
                     <cylinderGeometry args={[4, 6, 5, 16]} />
                     <meshStandardMaterial emissive="#a855f7" emissiveIntensity={2} color="#000" />
                </mesh>
                <Sparkles count={50} scale={120} color="#a855f7" size={20} speed={0.5} opacity={0.5} />
            </group>
        );
    }

    if (tier === 'ELITE') {
        // Elite: "Interceptor" - Needle nose, forward swept
        return (
            <group rotation={[0, Math.PI, 0]}>
                {/* Fuselage */}
                <mesh rotation={[Math.PI/2, 0, 0]}>
                    <coneGeometry args={[1.5, 12, 8]} />
                    <meshStandardMaterial color="#7f1d1d" roughness={0.4} metalness={0.6} />
                </mesh>
                {/* Wings */}
                <mesh position={[0, 0, 2]} rotation={[Math.PI/2, 0, 0]}>
                    <cylinderGeometry args={[4, 1, 0.5, 3]} /> 
                    <meshStandardMaterial color="#991b1b" />
                </mesh>
                <Trail width={2} length={15} color="#f97316" attenuation={(t) => t * t}>
                    <mesh position={[0, 0, 6]}><sphereGeometry args={[0.5]} /><meshBasicMaterial color="#f97316" /></mesh>
                </Trail>
            </group>
        );
    }

    // Standard: "Drone" - Sphere cockpit + Solar panels (TIE-ish)
    return (
        <group rotation={[0, Math.PI, 0]}>
            <mesh>
            ...
            <mesh rotation={[0, 0, Math.PI/2]}>
             ...
             <Trail width={1.5} length={8} color="#ef4444" attenuation={(t) => t * t}>
                <mesh position={[0, 0, 2]}><sphereGeometry args={[0.4]} /><meshBasicMaterial color="#ef4444" /></mesh>
            </Trail>
        </group>
    );
};
...
const TargetView: React.FC<{ item: Target; texture: Texture | null; playerPos: Vector3; camera: THREE.Camera }> = ({ item, playerPos, camera }) => {
    ...
    return (
        <group ref={ref} position={item.position}>
           <EnemyShipGeometry tier={item.tier} />
...
export const CombatSystem: React.FC<CombatSystemProps> = ({ 
  ...
  const spawnWave = (wave: number) => {
      ...
      if (wave === 1) {
          ...
          setCinematic({ active: true, type: 'BOSS_INTRO', targetPosition: bossPos.toArray(), message: "THE GILDED TYRANT", transmission: "End of the line, pilot." });
