...
import { SphereConfig, Transform } from '../../types';
...
export const CentralConstruct: React.FC<{ config: SphereConfig, transform: Transform }> = ({ config, transform }) => {
  ...
  useEffect(() => {
    const handleFeed = (e: CustomEvent) => {
      setFeedPulse(2.0); // High amplitude pulse
    };
    window.addEventListener('vizzy-feed' as any, handleFeed);
    return () => window.removeEventListener('vizzy-feed' as any, handleFeed);
  }, []);
  ...
  useFrame((state) => {
    // Pulse decay
    if (feedPulse > 0) {
      setFeedPulse(prev => Math.max(0, prev - 0.05));
    }
