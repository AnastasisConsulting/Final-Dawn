...
import PersonaApp from './features/persona/App';

type ModuleView = 'HOME' | 'GAME' | 'EDITOR' | 'PERSONA';
...
export default function App() {
  ...
  if (activeModule === 'PERSONA') {
    return (
      <div className="h-screen w-screen flex flex-col">
        <BackButton onClick={() => setActiveModule('HOME')} label="DISCONNECT LINK" />
        <div className="flex-1 overflow-hidden">
          <PersonaApp />
        </div>
      </div>
    ...
    <div className="h-screen w-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-900 via-[#020617] to-black flex items-center justify-center">
      ...
      <div className="max-w-4xl w-full p-8 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <MenuCard 
            title="PERSONA NEXUS" 
            desc="Neural Interface. Roleplay with AI Constructs."
            icon={BrainCircuit} 
            color="group-hover:text-emerald-400"
            border="group-hover:border-emerald-500/50"
            bg="group-hover:bg-emerald-950/20"
            onClick={() => setActiveModule('PERSONA')} 
          />
        </div>
