...
import type { GamePiece } from './pieces/BasePiece';
...
export class MoraleEngine {
  public static getInitialMorale(color: PieceColor): TeamMorale {
    return { color, totalConfidence: 50, boardControl: 0, threatPressure: 0, momentum: 0, posture: 'Balanced' };
  }
  ...
  public static updateHierarchy(activePieces: PieceMap, graveyard: PieceMap) {
    ...
    // compute and return a map of moraleWeight per piece (for those who are leaders)
    const moraleWeights = new Map<string, number>();
    activePieces.forEach(p => {
      // base self = 1 + active subordinate counts (as per spec)
      const support = getSupportSize(p.id); // counts subordinates recursively
      moraleWeights.set(p.id, 1 + support);
    });
  ...
  /**
   * Map the output of evaluation engine into TeamMorale (unchanged)
   */
