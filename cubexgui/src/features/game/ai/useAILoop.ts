...
export const useAILoop = (state: GameState, dispatch: React.Dispatch<Action>) => {
    useEffect(() => {
        ...
        if (state.gameMode === 'HvAI' && state.turn === 'black' && !state.pendingPromotion) {
            console.log("⚡ [AI LOOP] Black's Turn detected. Starting Thinking Sequence...");
            ...
            setTimeout(() => {
                ...
                const move = getAiMoveWithContext(state, state.difficulty);
