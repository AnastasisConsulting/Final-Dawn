// PATH: services/validation/worldValidator.ts
...
export function validateWorldState(
  ...
  // 6. Forbidden Strings (Blob/Data URL scan)
  // We explicitly EXCLUDE the textureUrl from this check because it is validly a blob URL at runtime.
  // The Export process handles converting this blob to a file, so it won't end up as text in the JSON.
  const stateClone = { ...state, textureUrl: "PLACEHOLDER_FOR_VALIDATION" };
  const jsonStr = JSON.stringify(stateClone);
  
  if (jsonStr.includes('blob:')) errors.push("State contains hidden blob: URLs (must be resolved before export)");
  if (jsonStr.includes('data:image')) errors.push("State contains base64 images (must be resolved before export)");
