// PATH: src/services/ids/idEnforcer.ts
...
export function normalizePlanetStateIds(worldId: string, state: PlanetState): PlanetState {
  ...
  state.metadata.civilizations.forEach((civ, civIdx) => {
    ...
    civ.cities.forEach(city => {
      ...
      // CRITICAL ORDER FIX: Process Regions BEFORE Governors/Lieutenants
      // This ensures idMap has the new Region IDs ready for Lieutenants to link to.
      ...
      if (city.governor) {
        ...
        city.governor.lieutenants.forEach(sub => {
          ...
          // Fix: Ensure Subordinate points to the NEW Region ID
          if (idMap[sub.regionId]) {
              sub.regionId = idMap[sub.regionId];
          }
        });
      }
    });
